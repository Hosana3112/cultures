...
@section('page_title', 'Modifier le Média')
@section('breadcrumb')
        ...
        <div class="card-body">
            <form action="{{ route('admin.medias.update', $media->id) }}" method="POST" enctype="multipart/form-data" id="media-form">
                ...
                <div class="row">
                            ...
                            <select class="form-control @error('type_media_id') is-invalid @enderror" 
                                ...
                                @foreach($typemedias as $type)
                                    <option value="{{ $type->id }}" 
                                            {{ old('type_media_id', $media->type_media_id) == $type->id ? 'selected' : '' }}>
                                        {{ $type->nom }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <!-- Fichier (optionnel pour la modification) -->
                        <div class="form-group mb-3">
                            ...
                            @if($media->chemin)
                                <div class="alert alert-info mt-2 p-2">
                                    <small>
                                        ...
                                        <strong>{{ $media->nom_original }}</strong>
                                        <br>
                                        <i class="fas fa-hdd"></i> Stocké dans: {{ $media->chemin }}
                                        @if($media->taille)
                                            <br>
                                            <i class="fas fa-weight"></i> Taille: {{ number_format($media->taille / 1024, 2) }} KB
                                        @endif
                                    </small>
                                </div>
                            @endif
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control @error('contenu_id') is-invalid @enderror" id="contenu_id" name="contenu_id" required>
                                ...
                                @foreach($contenus as $contenu)
                                    <option value="{{ $contenu->id }}" {{ old('contenu_id', $media->contenu_id) == $contenu->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <!-- Nom original -->
                        <div class="form-group mb-3">
                            <label for="nom_original" class="form-label">Nom d'affichage *</label>
                            <input type="text" class="form-control @error('nom_original') is-invalid @enderror" 
                                   id="nom_original" name="nom_original" 
                                   value="{{ old('nom_original', $media->nom_original) }}" 
                                   placeholder="Nom du fichier pour l'affichage" required>
                            @error('nom_original')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="Description du média, contexte, droits d'auteur...">{{ old('description', $media->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Validation côté client du fichier (seulement si un fichier est sélectionné)
    fileInput.addEventListener('change', function() {
        const selectedOption = typeSelect.options[typeSelect.selectedIndex];
        const typeName = selectedOption.textContent;
        const file = this.files[0];
        
        if (!file) return; // Pas de fichier sélectionné = pas de validation
