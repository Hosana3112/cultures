...
@section('page_title', 'Créer un Commentaire')
@section('breadcrumb')
...
<div class="container-fluid">
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ url('/admin/commentaires') }}" method="POST">
                @csrf
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control @error('user_id') is-invalid @enderror" id="user_id" name="user_id" required>
                                ...
                                @foreach($utilisateurs as $utilisateur)
                                    <option value="{{ $utilisateur->id }}" {{ old('user_id') == $utilisateur->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-12">
                        <div class="form-group mb-3">
                            ...
                            <textarea class="form-control @error('texte') is-invalid @enderror" 
                                      ...
                                      placeholder="Rédigez votre commentaire ici..." required>{{ old('texte') }}</textarea>
                            @error('texte')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror>
                        </div>
                                    ...
                                    @for($i = 1; $i <= 5; $i++)
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="note" id="note{{ $i }}" 
                                                   value="{{ $i }}" {{ old('note') == $i ? 'checked' : '' }} required>
